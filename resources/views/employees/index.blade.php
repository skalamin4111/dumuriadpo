<x-app-layout title="Employees">
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <form class="flex gap-2">
            <input class="field w-64" name="search" value="{{ request('search') }}" placeholder="Search employees">
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @foreach ($employees as $employee)
                        <tr class="table-row">
                            <td class="px-4 py-3"><p class="font-medium">{{ $employee->user->name }}</p><p class="text-xs text-slate-500">{{ $employee->user->email }}</p></td>
                            <td class="px-4 py-3">{{ $employee->department?->name ?? '-' }}</td>
                            <td class="px-4 py-3">{{ $employee->designation ?? '-' }}</td>
                            <td class="px-4 py-3">{{ $employee->phone ?? '-' }}</td>
                            <td class="px-4 py-3"><span class="badge bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200">{{ $employee->status }}</span></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div x-data="{open:false}" x-on:open-modal.window="open = $event.detail === 'employee-modal'" x-show="open" class="fixed inset-0 z-40 grid place-items-center bg-slate-950/60 p-4" style="display:none">
        <form method="POST" action="{{ route('employees.store') }}" enctype="multipart/form-data" class="card w-full max-w-2xl p-5">
            @csrf
