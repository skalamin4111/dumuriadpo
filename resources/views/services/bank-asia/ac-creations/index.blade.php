<x-app-layout title="Source of Fund Declarations (আয়ের উৎস ঘোষণা-পত্র)">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div>
            <a href="{{ route('bank-asia.ac-creations.create') }}" class="btn btn-primary inline-flex items-center gap-2">
    ...
    <form action="{{ route('bank-asia.ac-creations.index') }}" method="GET" class="surface p-4 mb-6">
        <div class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1 w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by customer name, NID, phone or account no..." class="field">
            </div>
            ...
            <div class="flex gap-2 w-full md:w-auto">
                ...
                <a href="{{ route('bank-asia.ac-creations.index') }}" class="btn btn-muted w-full md:w-auto px-4 text-center">Reset</a>
            </div>
        </div>
    </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($acCreations as $ac)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                            <td class="p-4 whitespace-nowrap">{{ $ac->date->format('Y-m-d') }}</td>
                            <td class="p-4">
                                @if($ac->account_type === 'new')
                            ...
                            <td class="p-4 font-bold text-slate-900 dark:text-white">
                                {{ $ac->applicant_name_bn }}
                            </td>
                            <td class="p-4 font-mono text-xs">{{ $ac->nid_number }}</td>
                            <td class="p-4 font-semibold">{{ $ac->mobile_number }}</td>
                            <td class="p-4">
                                <div class="text-xs">
                                    <span class="text-slate-400">A/C:</span> {{ $ac->account_number ?: 'N/A' }}
                                </div>
                                <div class="text-xs mt-0.5">
                                    <span class="text-slate-400">Cust ID:</span> {{ $ac->customer_id ?: 'N/A' }}
                                </div>
                            </td>
                            ...
                            <td class="p-4 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('bank-asia.ac-creations.show', $ac) }}" class="btn btn-muted py-1.5 px-3 text-xs flex items-center gap-1">
                                    ...
                                    <a href="{{ route('bank-asia.ac-creations.edit', $ac) }}" class="btn btn-muted py-1.5 px-3 text-xs flex items-center gap-1">
                                    ...
                                    <form action="{{ route('bank-asia.ac-creations.destroy', $ac) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this declaration?');" class="inline">
                                        @csrf
                                        @method('DELETE')
