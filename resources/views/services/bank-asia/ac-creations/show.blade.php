<x-app-layout title="আয়ের উৎস ঘোষণা-পত্র - Details">
    <style>
        ...
        .pdf-overlay-page.page-1 {
            background-image: url("{{ str_replace(['http:', 'https:'], '', asset('bank-asia/page-1.png')) }}");
        }

        .pdf-overlay-page.page-2 {
            background-image: url("{{ str_replace(['http:', 'https:'], '', asset('bank-asia/page-2.png')) }}");
        }
    ...
    <div x-data="{ activeTab: 'pdf' }">
        ...
        <div class="mb-6 flex items-center justify-between no-print">
            <div class="flex items-center gap-3">
                <a href="{{ route('bank-asia.ac-creations.index') }}" class="btn btn-muted flex items-center gap-2">
            ...
            <div class="flex gap-3">
                <a href="{{ route('bank-asia.ac-creations.edit', $acCreation) }}" class="btn btn-muted flex items-center gap-2">
        ...
        <div class="print-mode-wrapper" :class="activeTab === 'pdf' ? 'print-pdf-mode' : 'print-digital-mode'">
            ...
            {{-- OPTION 1: PDF OVERLAY PRINT --}}
            <div class="print-pdf-only" x-show="activeTab === 'pdf'">
                {{-- PAGE 1: Original Scanned page 1 background with overlay --}}
                <div class="pdf-overlay-page page-1">
                    {{-- Date --}}
                    <div class="overlay-field" style="top: 14.4%; left: 17.0%;">
                        {{ $acCreation->date->format('d-m-Y') }}
                    </div>

                    {{-- Checkboxes --}}
                    @if($acCreation->account_type === 'new')
                    ...
                    <div class="absolute flex items-center justify-center" style="top: 77.0%; left: 15.0%; width: 150px; height: 65px;">
                        @if($acCreation->applicant_signature_path)
                            <img src="{{ str_replace(['http:', 'https:'], '', asset('storage/' . $acCreation->applicant_signature_path)) }}" class="max-w-full max-h-full object-contain">
                        @endif
                    </div>

                    {{-- CSO Verification signature text overlay --}}
                    <div class="overlay-field" style="top: 78.5%; left: 78.5%; line-height: 1.4;">
                        <div style="display:none;">নাম: {{ $acCreation->agent_name ?? 'মোঃ আলামিন' }}</div>
                        <div style="display:none;">Designation: {{ $acCreation->agent_designation ?? 'সি এস ও' }}</div>
                        <div>{{ $acCreation->agent_mobile ?? '01955801666' }}</div>
                    </div>
                </div>

                {{-- PAGE 2: Original Scanned page 2 background with overlay --}}
                <div class="pdf-overlay-page page-2">
                    {{-- Date --}}
                    <div class="overlay-field" style="top: 13.1%; left: 19.0%; font-size:16px;">
                        {{ $acCreation->date->format('d') }}<span style="margin:0 15px"></span>{{ $acCreation->date->format('m') }}<span style="margin:0 16px"></span>{{ $acCreation->date->format('Y') }}
                    </div>
                    ...
                    {{-- Page 2 Signature box scan overlay --}}
            ...
            {{-- OPTION 2: CLEAN DIGITAL HTML VIEW --}}
            <div class="print-digital-only" x-show="activeTab === 'digital'">
                {{-- Page 1: আয়ের উৎস ঘোষণা-পত্র --}}
                <div class="print-page flex flex-col justify-between">
                    <div>
                        ...
                        <div class="flex justify-between items-center mb-6 text-sm">
                            <div>
                                <strong>তারিখ:</strong> <span class="underline-dotted px-2">{{ $acCreation->date->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex gap-6">
                                <div>
                                    <span class="checkbox-box">{!! $acCreation->account_type === 'new' ? '&#10003;' : '&nbsp;' !!}</span>
                                    <span>নতুন হিসাব</span>
                                </div>
                                <div>
                                    <span class="checkbox-box">{!! $acCreation->account_type === 'dormant' ? '&#10003;' : '&nbsp;' !!}</span>
                        ...
                        <div class="space-y-6 bn-text-justify text-base leading-relaxed text-black">
                            <p>
                                আমি <span class="underline-dotted px-2">{{ $acCreation->applicant_name_bn }}</span>, 
                                পিতা/স্বামীর নামঃ <span class="underline-dotted px-2">{{ $acCreation->father_name }}</span>, 
                                মাতার নামঃ <span class="underline-dotted px-2">{{ $acCreation->mother_name }}</span>, 
                                জাতীয় পরিচয়পত্র/পাসপোর্ট/জন্ম নিবন্ধন নম্বরঃ <span class="underline-dotted px-2 font-mono">{{ $acCreation->nid_number }}</span>, 
                                বর্তমান ঠিকানা: <span class="underline-dotted px-2">{{ $acCreation->present_address }}</span>, 
                                আপনার <span class="underline-dotted px-2">{{ $acCreation->outlet_name_address }}</span> এজেন্ট আউটলেটের সেবা গ্রহণকারী।
                            </p>

                            <p>
                                বর্তমানে আমার পেশা <span class="underline-dotted px-2">{{ $acCreation->occupation }}</span>। 
                                আমার পেশার প্রমাণস্বরূপ কোন দাপ্তরিক ডকুমেন্ট নেই। আমার আয়ের উৎস মূলত 
                                <span class="underline-dotted px-2">{{ $acCreation->source_of_funds }}</span> 
                                যেখান থেকে আমার আনুমানিক মাসিক আয় <span class="underline-dotted px-2 font-mono">{{ number_format($acCreation->monthly_income, 0) }}</span> টাকা।
                            </p>
                        ...
                        <div class="grid grid-cols-2 gap-8 text-center text-sm">
                            <div class="flex flex-col items-center justify-end">
                                <div class="h-16 flex items-center justify-center mb-2">
                                    @if($acCreation->applicant_signature_path)
                                        <img src="{{ str_replace(['http:', 'https:'], '', asset('storage/' . $acCreation->applicant_signature_path)) }}" alt="Signature" class="max-h-16 object-contain">
                                    @else
                            ...
                            <div class="flex flex-col items-center justify-end">
                                <div class="h-16 flex flex-col justify-end text-left text-xs text-slate-700 w-56 pb-2">
                                    <div>নাম: {{ $acCreation->agent_name ?? 'মোঃ আলামিন' }}</div>
                                    <div>পদবী: {{ $acCreation->agent_designation ?? 'সি এস ও' }}</div>
                                    <div>মোবাইল: {{ $acCreation->agent_mobile ?? '01955801666' }}</div>
                                </div>
                ...
                {{-- Page 2: হিসাব পরিচালনা ও লেনদেন সংক্রান্ত সচেতনতা --}}
                <div class="print-page flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-start mb-6 border-b border-black pb-4">
                            <div>
                    ...
                    <div class="mt-8">
                        <table class="awareness-table text-black">
                            <tr>
                                ...
                                <td>{{ $acCreation->applicant_name_bn }}</td>
                                <td rowspan="4" class="text-center align-middle" style="width: 35%;">
                                    <div class="flex flex-col items-center justify-center h-28">
                                        @if($acCreation->applicant_signature_path)
                                            <img src="{{ str_replace(['http:', 'https:'], '', asset('storage/' . $acCreation->applicant_signature_path)) }}" alt="Signature" class="max-h-16 object-contain mb-2">
                                        @else
                            ...
                            <tr>
                                ...
                                <td class="font-mono">{{ $acCreation->customer_id ?? '---' }}</td>
                            </tr>
                            <tr>
                                <td class="label-cell">মোবাইল নম্বর</td>
                                <td class="font-mono">{{ $acCreation->mobile_number }}</td>
                            </tr>
                            <tr>
                                <td class="label-cell">হিসাব নম্বর (যদি থাকে)</td>
                                <td class="font-mono">{{ $acCreation->account_number ?? '---' }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
