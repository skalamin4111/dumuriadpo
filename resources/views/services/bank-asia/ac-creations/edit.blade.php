<x-app-layout title="আয়ের উৎস ঘোষণা-পত্র - Edit Declaration">
    <div class="mb-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('bank-asia.ac-creations.show', $acCreation) }}" class="btn btn-muted px-2.5">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Modify details of the Source of Fund declaration for {{ $acCreation->applicant_name_bn }}.</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('bank-asia.ac-creations.update', $acCreation) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <section class="surface p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <input type="date" name="date" class="field" value="{{ old('date', $acCreation->date->format('Y-m-d')) }}" required>
                </div>
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="inline-flex items-center">
                            <input type="radio" name="account_type" value="new" {{ old('account_type', $acCreation->account_type) === 'new' ? 'checked' : '' }} class="text-primary-600 focus:ring-primary-500">
                            <span class="ml-2 text-sm text-slate-700 dark:text-slate-300">নতুন (New)</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="account_type" value="dormant" {{ old('account_type', $acCreation->account_type) === 'dormant' ? 'checked' : '' }} class="text-primary-600 focus:ring-primary-500">
                ...
                <div>
                    ...
                    <input type="text" name="account_number" class="field" placeholder="হিসাব নং (যদি থাকে)" value="{{ old('account_number', $acCreation->account_number) }}">
                </div>
            </div>
        </section>
        ...
        <section class="surface p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <input type="text" name="applicant_name_bn" class="field" placeholder="গ্রাহকের সম্পূর্ণ নাম" value="{{ old('applicant_name_bn', $acCreation->applicant_name_bn) }}" required>
                </div>
                <div>
                    ...
                    <input type="text" name="father_name" class="field" placeholder="পিতা অথবা স্বামীর নাম" value="{{ old('father_name', $acCreation->father_name) }}" required>
                </div>
                <div>
                    ...
                    <input type="text" name="mother_name" class="field" placeholder="মাতার নাম" value="{{ old('mother_name', $acCreation->mother_name) }}" required>
                </div>
                <div>
                    ...
                    <input type="text" name="nid_number" class="field" placeholder="NID / Passport / Birth Reg" value="{{ old('nid_number', $acCreation->nid_number) }}" required>
                </div>
                <div>
                    ...
                    <input type="text" name="mobile_number" class="field" placeholder="মোবাইল নম্বর" value="{{ old('mobile_number', $acCreation->mobile_number) }}" required>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">কাস্টমার আইডি (Customer ID)</label>
                    <input type="text" name="customer_id" class="field" placeholder="কাস্টমার আইডি" value="{{ old('customer_id', $acCreation->customer_id) }}">
                </div>
            </div>
            <div class="grid gap-5 sm:grid-cols-1 mt-4">
                <div>
                    ...
                    <textarea name="present_address" rows="2" class="field" placeholder="গ্রাম, ডাকঘর, উপজেলা, জেলা" required>{{ old('present_address', $acCreation->present_address) }}</textarea>
                </div>
            </div>
        </section>
        ...
        <section class="surface p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <div>
                    ...
                    <input type="text" name="occupation" class="field" placeholder="যেমন: ব্যবসা, গৃহিণী, কৃষি" value="{{ old('occupation', $acCreation->occupation) }}" required>
                </div>
                <div>
                    ...
                    <input type="text" name="source_of_funds" class="field" placeholder="যেমন: কৃষি কাজ, দোকানের আয়, প্রবাসী অর্থ" value="{{ old('source_of_funds', $acCreation->source_of_funds) }}" required>
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="monthly_income" class="field" placeholder="টাকা" value="{{ old('monthly_income', $acCreation->monthly_income) }}" required>
                </div>
            </div>
        </section>
        ...
        <section class="surface p-6">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                <div>
                    ...
                    <input type="text" name="outlet_name_address" class="field" value="{{ old('outlet_name_address', $acCreation->outlet_name_address) }}" required>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">যাচাইকারী এজেন্ট/সিএসও এর নাম</label>
                    <input type="text" name="agent_name" class="field" value="{{ old('agent_name', $acCreation->agent_name) }}">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">পদবী (Designation)</label>
                    <input type="text" name="agent_designation" class="field" value="{{ old('agent_designation', $acCreation->agent_designation) }}">
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">মোবাইল নম্বর (Agent/CSO)</label>
                    <input type="text" name="agent_mobile" class="field" value="{{ old('agent_mobile', $acCreation->agent_mobile) }}">
                </div>
            </div>
            <div class="grid gap-5 sm:grid-cols-2 mt-4">
                <div>
                    ...
                    <select name="status" class="field" required>
                        <option value="pending" {{ old('status', $acCreation->status) === 'pending' ? 'selected' : '' }}>Pending (পেন্ডিং)</option>
                        <option value="submitted" {{ old('status', $acCreation->status) === 'submitted' ? 'selected' : '' }}>Submitted (দাখিলকৃত)</option>
                        <option value="approved" {{ old('status', $acCreation->status) === 'approved' ? 'selected' : '' }}>Approved (অনুমোদিত)</option>
                        <option value="rejected" {{ old('status', $acCreation->status) === 'rejected' ? 'selected' : '' }}>Rejected (প্রত্যাখ্যাত)</option>
                    </select>
                </div>
                
                {{-- Applicant Signature Upload --}}
                <div x-data="{ preview: '{{ $acCreation->applicant_signature_path ? asset('storage/' . $acCreation->applicant_signature_path) : '' }}' }" class="flex flex-col items-center p-4 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <div class="flex gap-4 justify-end">
            <a href="{{ route('bank-asia.ac-creations.show', $acCreation) }}" class="btn btn-muted px-6">Cancel</a>
