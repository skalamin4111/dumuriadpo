                    ...
                    @forelse ($tpUpdates as $update)
                        ...
                        <tr class="block sm:table-row bg-white dark:bg-slate-900 shadow-sm sm:shadow-none border border-slate-200 dark:border-slate-800 sm:border-0 rounded-xl sm:rounded-none p-4 sm:p-0 transition hover:bg-slate-50 dark:hover:bg-slate-800">
                            ...
                            <td class="flex sm:table-cell items-center gap-3 sm:gap-0 sm:px-4 py-2 sm:py-3 font-medium whitespace-nowrap">
                                ...
                                <span class="text-slate-900 dark:text-slate-100">{{ $update->account_name }}</span>
                            </td>
                            <td class="flex sm:table-cell items-center gap-3 sm:gap-0 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                                <span class="sm:hidden font-medium text-xs text-slate-500 w-24 shrink-0">A/C Number:</span>
                                <span>{{ $update->account_number }}</span>
                            </td>
                                    ...
                                    @if(count($regularTypes) > 0)
                                        <div>
                                            ...
                                            @if(count($regularFreqs) > 0)
                                                <div class="mt-1 text-[11px] font-medium text-slate-500 dark:text-slate-400">{{ implode(' & ', $regularFreqs) }}</div>
                                            @endif
                                        </div>
                                    @endif
                                    ...
                                    @if(count($oneTimeTypes) > 0)
                                        <div>
                                            ...
                                            @if(count($oneTimeFreqs) > 0)
                                                <div class="mt-1 text-[11px] font-medium text-slate-500 dark:text-slate-400">{{ implode(' & ', $oneTimeFreqs) }}</div>
                                            @endif
                                        </div>
                                    @endif
                            ...
                            <td class="flex sm:table-cell items-center gap-3 sm:gap-0 sm:px-4 py-2 sm:py-3 font-medium align-top whitespace-nowrap">
                                ...
                                <span>{{ $amount > 0 ? '৳ ' . number_format($amount) : '-' }}</span>
                            </td>
                            <td class="block sm:table-cell sm:px-4 pt-4 pb-2 sm:py-3 mt-3 sm:mt-0 border-t border-slate-200 sm:border-0 dark:border-slate-700 sm:text-right">
                                <div class="flex items-center justify-between sm:justify-end gap-2">
                                    <a href="{{ route('bank-asia.tp-updates.show', $update) }}" class="btn btn-muted px-3 py-1.5 flex-1 sm:flex-none justify-center gap-1.5" title="View / Print">
                                    ...
                                    <a href="{{ route('bank-asia.tp-updates.edit', $update) }}" class="btn btn-muted px-3 py-1.5 flex-1 sm:flex-none justify-center gap-1.5" title="Edit">
                                    ...
                                    <form action="{{ route('bank-asia.tp-updates.destroy', $update) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this TP update?');" class="flex-1 sm:flex-none block">
                                        @csrf
                                        @method('DELETE')
