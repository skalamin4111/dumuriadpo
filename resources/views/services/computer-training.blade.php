...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <button type="button" @click="student = { id: null, name: '', name_bn: '', father_name: '', mother_name: '', date_of_birth: '', nid_or_birth_reg: '', nationality: 'Bangladeshi', gender: '', marital_status: '', religion: '', educational_qualifications: [{}, {}], course: '', duration: '', session: '', admission_date: '{{ now()->toDateString() }}', student_id: '', guardian_name: '', guardian_phone: '', status: 'admitted', notes: '', address: '', phone: '', email: '', batch_id: '', seat_number: '' }" class="btn btn-primary shrink-0">Add New Student</button>
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        @forelse ($students as $student)
                            <tr class="table-row cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50" @click="viewStudent = {{ Js::from($student) }}">
                            <td class="px-4 py-3 font-medium">
                                {{ $student->name }}
                                <span class="block text-xs text-slate-500">
                                    {{ $student->student_id }} 
                                    @if($student->batch) &bull; <span class="font-medium text-teal-600">{{ $student->batch->name }} (Seat: {{ $student->seat_number ?? 'N/A' }})</span> @endif
                                </span>
                            </td>
...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
            ...
            <form method="GET" action="{{ url()->current() }}" class="flex flex-col sm:flex-row gap-3 mb-6 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <input type="hidden" name="tab" value="attendance">
                
                <div class="flex-1">
                    <input type="date" name="attendance_date" value="{{ request('attendance_date') }}" class="field w-full" placeholder="Date">
                </div>
                ...
                <div class="flex-1">
                    <select name="attendance_course" class="field w-full">
                        ...
                        @foreach($courses as $c)
                            <option value="{{ $c }}" {{ request('attendance_course') == $c ? 'selected' : '' }}>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-[2]">
                    <div class="relative">
                        <input type="text" name="attendance_search" value="{{ request('attendance_search') }}" class="field w-full pl-10" placeholder="Search name, phone, ID...">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(request('attendance_date') || request('attendance_course') || request('attendance_batch') || request('attendance_search'))
                        <a href="{{ url()->current() }}?tab=attendance" class="btn bg-slate-200 text-slate-700 hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 px-4 py-2">Clear</a>
                    @endif
                </div>
            </form>
