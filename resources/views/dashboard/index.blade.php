<x-app-layout title="Dashboard">
    ...
    <div class="mb-8 relative overflow-hidden rounded-2xl border border-slate-200/50 bg-white/40 p-8 backdrop-blur-xl shadow-sm dark:border-slate-800/50 dark:bg-slate-900/40">
        ...
        <div class="relative z-10">
            ...
            <p class="mt-2 text-slate-500 dark:text-slate-400 max-w-2xl text-lg">
                Here's a detailed overview of your operations today. You have <strong class="text-teal-600 dark:text-teal-400">{{ $stats['pending_tasks'] }} tasks</strong> awaiting approval.
            </p>
        </div>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        @foreach ([
        ...
        ] as [$label, $value, $hint, $path, $gradient, $badgeClass, $trend])
            <div class="group relative overflow-hidden rounded-2xl border border-slate-200/60 bg-white p-6 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/80 backdrop-blur-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-slate-300 dark:hover:border-slate-700">
                <div class="absolute top-0 right-0 -mr-8 -mt-8 h-32 w-32 rounded-full bg-gradient-to-br {{ $gradient }} opacity-10 blur-2xl group-hover:opacity-20 transition-opacity duration-500"></div>
                
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-sm font-semibold text-slate-500 uppercase tracking-wider dark:text-slate-400">{{ $label }}</p>
                        <div class="mt-4 flex items-baseline gap-2">
                            <p class="text-4xl font-black tracking-tight text-slate-900 dark:text-white">{{ $value }}</p>
                            <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold {{ $badgeClass }}">
                                {{ $trend }}
                            </span>
                        </div>
                    </div>
                    <div class="flex size-12 items-center justify-center rounded-xl bg-gradient-to-br {{ $gradient }} shadow-inner shadow-white/20">
                        <svg class="size-6 text-white drop-shadow-md" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="{{ $path }}"/></svg>
                    </div>
                </div>
                <div class="mt-5 flex items-center text-sm">
                    <span class="text-slate-500 font-medium dark:text-slate-400">{{ $hint }}</span>
                </div>
            </div>
        @endforeach
    </div>
    ...
    <div class="mt-8 grid gap-8 xl:grid-cols-[1.5fr_1fr]">
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <span class="inline-flex items-center gap-1.5 rounded-lg bg-slate-50 px-3 py-1.5 text-sm font-semibold text-slate-700 ring-1 ring-inset ring-slate-200 dark:bg-slate-800/50 dark:text-slate-300 dark:ring-slate-700">
                        ...
                        {{ $stats['completed_tasks'] }} Total Completed
                    </span>
                </div>
            </div>
        ...
        <div class="flex flex-col rounded-2xl border border-slate-200/60 bg-white p-7 shadow-sm dark:border-slate-800/80 dark:bg-slate-900/80 backdrop-blur-sm">
            <div class="mb-6 flex items-center justify-between">
                ...
                <a href="{{ route('tasks.index') }}" class="inline-flex items-center gap-1 text-sm font-semibold text-teal-600 hover:text-teal-700 dark:text-teal-400 dark:hover:text-teal-300 transition-colors">
                    View all 
            ...
            <div class="flex-1 space-y-5 overflow-y-auto pr-2">
                @forelse ($stats['recent_tasks'] as $task)
                    <a href="{{ route('tasks.show', $task) }}" class="group flex items-start gap-4 rounded-xl p-3 -mx-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/40">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center justify-between gap-2">
                                <p class="truncate text-sm font-bold text-slate-900 group-hover:text-teal-600 transition-colors dark:text-white dark:group-hover:text-teal-400">{{ $task->title }}</p>
