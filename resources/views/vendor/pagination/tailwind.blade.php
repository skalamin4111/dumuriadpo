                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-700 bg-white border-y border-l border-slate-200 cursor-default leading-5 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300">{{ $element }}</span>
                            </span>
                        @endif
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-bold text-white bg-teal-600 border border-teal-600 cursor-default leading-5 dark:bg-teal-500 dark:border-teal-500 shadow-inner">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-slate-700 bg-white border-y border-l border-slate-200 leading-5 hover:text-teal-600 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-teal-500/20 active:bg-slate-100 transition ease-in-out duration-150 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:text-teal-400 dark:hover:bg-slate-800/80" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
