<x-app-layout title="Customers">
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <form class="flex gap-2"><input class="field w-64" name="search" value="{{ request('search') }}" placeholder="Search customers"><button class="btn btn-muted"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M7 12h10M10 18h4"/></svg>Filter</button></form>
        @can('create customers')<button class="btn btn-primary" x-data x-on:click="$dispatch('open-modal', 'customer-modal')"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>Add customer</button>@endcan
    </div>
            ...
            <article class="surface p-4 transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-md dark:hover:border-teal-900">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex min-w-0 gap-3">
                        ...
                        <div class="min-w-0"><h2 class="truncate font-semibold">{{ $customer->name }}</h2><p class="truncate text-sm text-slate-500">{{ $customer->company_name ?? $customer->email }}</p></div>
                    </div>
                    <span class="badge bg-teal-50 text-teal-700 dark:bg-teal-950 dark:text-teal-200">{{ $customer->type }}</span>
                </div>
                <p class="mt-4 text-sm text-slate-600 dark:text-slate-300">{{ $customer->phone ?? 'No phone' }}</p>
                <p class="mt-1 text-sm text-slate-500">{{ $customer->tasks_count }} tasks</p>
            </article>
        @endforeach
    </div>
    ...
    <div x-data="{open:false}" x-on:open-modal.window="open = $event.detail === 'customer-modal'" x-show="open" class="fixed inset-0 z-40 grid place-items-center bg-slate-950/60 p-4" style="display:none">
        <form method="POST" action="{{ route('customers.store') }}" class="card w-full max-w-2xl p-5">
            @csrf
