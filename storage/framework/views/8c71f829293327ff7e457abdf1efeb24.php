...
<div class="flex flex-col gap-4 sm:flex-row sm:items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800">
            ...
            <form method="GET" action="<?php echo e(url()->current()); ?>" class="flex flex-col lg:flex-row lg:items-center gap-3 bg-slate-50 dark:bg-slate-900/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800/60 mb-6 w-full min-w-0 overflow-hidden">
                ...
                <div class="relative flex-[2] w-full min-w-0">
                    ...
                    <input type="text" name="attendance_search" value="<?php echo e(request('attendance_search')); ?>" placeholder="Search name, phone, ID..." class="w-full max-w-full pl-10 pr-4 py-2 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all placeholder:text-slate-400 dark:text-slate-200 shadow-sm">
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:flex lg:flex-[3] lg:items-center min-w-0">
                    <div class="w-full lg:flex-1 min-w-0">
                        <input type="date" name="attendance_date" value="<?php echo e(request('attendance_date')); ?>" class="w-full max-w-full py-2 px-3 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all dark:text-slate-200 shadow-sm">
                    </div>
                    ...
                    <div class="flex items-center gap-2 w-full lg:w-auto min-w-0">
                        ...
                        <?php if(request('attendance_date') || request('attendance_course') || request('attendance_batch') || request('attendance_search')): ?>
                            <a href="<?php echo e(url()->current()); ?>?tab=attendance" class="flex-1 lg:flex-none justify-center text-sm font-medium text-rose-500 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 px-3 py-2 rounded-lg transition-colors flex items-center gap-1.5 shrink-0">
                                <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                Clear
                            </a>
                        <?php endif; ?>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $attendanceRecords; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attendance): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <article class="surface p-4 hover:shadow-md transition-shadow relative overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer"
                             @click="viewStudentAttendance = {
                                 id: <?php echo e($attendance->id); ?>,
                                 attendance_date: '<?php echo e($attendance->attendance_date->toDateString()); ?>',
                                 attendance_date_formatted: '<?php echo e($attendance->attendance_date->format('l, M j, Y')); ?>',
                                 ...
                                 student: {
                                     ...
                                     phone: '<?php echo e($attendance->student?->phone ?? 'N/A'); ?>',
                                     course: '<?php echo e(addslashes($attendance->student?->course ?? 'N/A')); ?>',
                                     present_count: <?php echo e($attendance->student?->present_count ?? 0); ?>,
                                     absent_count: <?php echo e($attendance->student?->absent_count ?? 0); ?>,
                                     late_count: <?php echo e($attendance->student?->late_count ?? 0); ?>,
                                     total_marks: <?php echo e($attendance->student?->total_marks ?? 0); ?>,
                                     batch: {
                                         name: '<?php echo e($attendance->student?->batch?->name ?? 'Unassigned'); ?>'
                                     }
                                 }
                             }">
                        ...
                        <div class="flex items-center gap-4 flex-1">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-1 text-sm text-slate-500">
                                    ...
                                    <?php if($attendance->student?->phone): ?>
                                        <div class="flex items-center gap-1 text-slate-500 dark:text-slate-400">
                                            <svg class="size-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                            <span><?php echo e($attendance->student->phone); ?></span>
                                        </div>
                                    <?php endif; ?>
                                ...
                                <?php if($attendance->remarks): ?>
                                    <div class="mt-2 flex">
                                        <span class="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-500/10 px-2.5 py-1 rounded-md border border-rose-100 dark:border-rose-950/30 shadow-sm">
                                            ...
                                            Absent Note: <?php echo e($attendance->remarks); ?>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button type="button" @click.stop="editAttendance = {
                                    ...
                                    student: {
                                        name: '<?php echo e(addslashes($attendance->student?->name ?? 'N/A')); ?>',
                                        batch: {
                                            name: '<?php echo e($attendance->student?->batch?->name ?? ''); ?>'
                                        }
                                    }
        ...
        <section x-show="tab === 'fees'" class="grid gap-5 xl:grid-cols-[22rem_1fr]" x-data="{ 
            feeCourse: '<?php echo e(request('fee_course')); ?>', 
            feeBatch: '<?php echo e(request('fee_batch')); ?>', 
            feeStudent: '',
            totalAmount: '3000', 
            paidAmount: '3000',
            feeType: 'Admission',
            status: 'paid',
            dueDate: '<?php echo e(now()->addMonth()->toDateString()); ?>',
            paidAtDate: '<?php echo e(now()->toDateString()); ?>',
            feeViewMode: 'matrix',
            ...
            bulkPaymentDate: '<?php echo e(now()->toDateString()); ?>',
            ...
            fetchBulkFeeStudents() {
                ...
                let url = '<?php echo e(route('computer-training.students-fees')); ?>?fee_type=' + encodeURIComponent(this.bulkFeeType) + '&batch_id=' + encodeURIComponent(this.bulkBatchId) + '&course=' + encodeURIComponent(this.bulkCourse);
                fetch(url)
                    .then(res => res.json())
                    .then(data => {
                ...
                <div class="surface p-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                    <form method="GET" class="flex flex-wrap items-center gap-2 flex-1">
                        ...
                        <div class="relative flex-1 min-w-[130px]">
                            <input type="text" name="fee_search" value="<?php echo e(request('fee_search')); ?>" placeholder="Search name/ID/phone" class="field py-1.5 px-3 text-xs w-full">
                        </div>
                        
                        <select name="fee_course" class="field py-1.5 px-3 text-xs w-auto" onchange="this.form.submit()">
                            <option value="">All Courses</option>
                            <?php $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($c); ?>" <?php echo e(request('fee_course') === $c ? 'selected' : ''); ?>><?php echo e($c); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>
                        ...
                        <select name="fee_batch" class="field py-1.5 px-3 text-xs w-auto" onchange="this.form.submit()">
                            <option value="">All Batches</option>
                            <?php $__currentLoopData = \App\Models\ComputerTrainingBatch::orderBy('name')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($b->id); ?>" <?php echo e(request('fee_batch') == $b->id ? 'selected' : ''); ?>><?php echo e($b->name); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </select>

                        <select class="field py-1.5 px-3 text-xs w-auto" name="fee_status" onchange="this.form.submit()">
                            <option value="">All Statuses</option>
                            <option value="paid" <?php echo e(request('fee_status') === 'paid' ? 'selected' : ''); ?>>Fully Paid</option>
                            <option value="due" <?php echo e(request('fee_status') === 'due' ? 'selected' : ''); ?>>Has Due</option>
                        </select>
                        
                        <button type="submit" class="btn btn-secondary py-1.5 px-3 text-xs">Filter</button>
                        <?php if(request('fee_search') || request('fee_course') || request('fee_batch') || request('fee_status')): ?>
                            <a href="<?php echo e(route('services.show', ['service' => 'computer-training', 'tab' => 'fees'])); ?>" class="text-xs text-rose-500 hover:underline">Clear</a>
                        <?php endif; ?>
                    </form>
                ...
                <div x-show="feeViewMode === 'matrix'" class="surface overflow-hidden">
                    <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/40">
                        ...
                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-teal-100 text-teal-800 dark:bg-teal-900/50 dark:text-teal-300">
                            Total Students: <?php echo e($studentFeeRecords->total()); ?>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            ...
                            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                                <?php $__empty_1 = true; $__currentLoopData = $studentFeeRecords; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <?php
                                    ...
                                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                                        <td class="p-3">
                                            <div class="font-bold text-slate-900 dark:text-slate-100 text-sm"><?php echo e($st->name); ?></div>
                                            <div class="text-xs text-slate-500 font-mono flex items-center gap-1.5 mt-0.5">
                                                <span>ID: <?php echo e($st->student_id ?? $st->id); ?></span>
                                                <?php if($st->batch): ?>
                                                    · <span class="text-teal-600 dark:text-teal-400 font-sans font-medium"><?php echo e($st->batch->name); ?></span>
                                                <?php endif; ?>
                                            </div>
                                            <?php if($st->phone): ?>
                                                <div class="text-[11px] text-slate-400 font-mono mt-0.5"><?php echo e($st->phone); ?></div>
                                            <?php endif; ?>
                                        </td>

                                        <!-- Admission Fee -->
                                        <td class="p-3 text-center">
                                            <span class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-semibold <?php echo e($admission['badge']); ?>">
                                                <?php echo e($admission['label']); ?>

                                            </span>
                                            <?php if($admission['amount'] > 0): ?>
                                                <div class="text-[11px] text-slate-500 mt-1 font-mono">
                                                    ৳ <?php echo e(number_format($admission['paid'], 0)); ?> / <?php echo e(number_format($admission['amount'], 0)); ?>
                                        ...
                                        <!-- Registration Fee -->
                                        <td class="p-3 text-center">
                                            <span class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-semibold <?php echo e($registration['badge']); ?>">
                                                <?php echo e($registration['label']); ?>

                                            </span>
                                            <?php if($registration['amount'] > 0): ?>
                                                <div class="text-[11px] text-slate-500 mt-1 font-mono">
                                                    ৳ <?php echo e(number_format($registration['paid'], 0)); ?> / <?php echo e(number_format($registration['amount'], 0)); ?>
                                        ...
                                        <!-- Form Fillup -->
                                        <td class="p-3 text-center">
                                            <span class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-semibold <?php echo e($formFillup['badge']); ?>">
                                                <?php echo e($formFillup['label']); ?>

                                            </span>
                                            <?php if($formFillup['amount'] > 0): ?>
                                                <div class="text-[11px] text-slate-500 mt-1 font-mono">
                                                    ৳ <?php echo e(number_format($formFillup['paid'], 0)); ?> / <?php echo e(number_format($formFillup['amount'], 0)); ?>
                                        ...
                                        <!-- Exam Fees -->
                                        <td class="p-3 text-center">
                                            <span class="inline-block px-2.5 py-0.5 rounded-full text-[11px] font-semibold <?php echo e($exam['badge']); ?>">
                                                <?php echo e($exam['label']); ?>

                                            </span>
                                            <?php if($exam['amount'] > 0): ?>
                                                <div class="text-[11px] text-slate-500 mt-1 font-mono">
                                                    ৳ <?php echo e(number_format($exam['paid'], 0)); ?> / <?php echo e(number_format($exam['amount'], 0)); ?>
                                        ...
                                        <td class="p-3 text-right">
                                            <button type="button" @click="selectStudentForFee('<?php echo e($st->id); ?>', '<?php echo e(addslashes($st->course)); ?>', '<?php echo e($st->batch_id); ?>')" class="btn btn-secondary text-xs px-2.5 py-1 inline-flex items-center gap-1 hover:bg-teal-50 dark:hover:bg-teal-950/40 hover:text-teal-600 transition">
                    ...
                    <div class="p-4 border-t border-slate-200 dark:border-slate-800">
                        <?php echo e($studentFeeRecords->appends(['tab' => 'fees', 'fee_course' => request('fee_course'), 'fee_batch' => request('fee_batch'), 'fee_status' => request('fee_status'), 'fee_search' => request('fee_search')])->links()); ?>
            ...
            <template x-teleport="body">
                <div x-show="showBulkFeeModal" 
                    ...
                    <div class="w-full max-w-5xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 relative flex flex-col max-h-[90vh]" @click.away="closeBulkFeeModal()">
                        ...
                        <form method="POST" action="<?php echo e(route('computer-training.fees.bulk')); ?>" @submit="handleBulkFeeSubmit($event)" class="flex flex-col overflow-hidden h-full">
                            <?php echo csrf_field(); ?>
                    ...
                    <form method="GET" action="<?php echo e(url()->current()); ?>" class="flex flex-col sm:flex-row items-center w-full bg-white dark:bg-slate-950">
                        ...
                        <div class="flex items-center w-full sm:w-auto pl-3 pr-2 py-2 sm:py-0 border-t sm:border-t-0 border-slate-200 dark:border-slate-800">
                            <select name="marketing_status" class="bg-transparent border-0 text-sm font-medium text-slate-600 dark:text-slate-300 focus:ring-0 outline-none cursor-pointer hover:text-slate-900 dark:hover:text-white transition w-full sm:w-auto [&>option]:dark:bg-slate-900" onchange="this.form.submit()">
                                <option value="">All Statuses</option>
                                <option value="new" <?php echo e(request('marketing_status') === 'new' ? 'selected' : ''); ?>>New</option>
                                <option value="contacting" <?php echo e(request('marketing_status') === 'contacting' ? 'selected' : ''); ?>>Contacting</option>
                                <option value="interested" <?php echo e(request('marketing_status') === 'interested' ? 'selected' : ''); ?>>Interested</option>
                                <option value="admitted" <?php echo e(request('marketing_status') === 'admitted' ? 'selected' : ''); ?>>Admitted</option>
                                <option value="not interested" <?php echo e(request('marketing_status') === 'not interested' ? 'selected' : ''); ?>>Not Interested</option>
                            </select>
                            ...
                            <select name="marketing_source" class="bg-transparent border-0 text-sm font-medium text-slate-600 dark:text-slate-300 focus:ring-0 outline-none cursor-pointer hover:text-slate-900 dark:hover:text-white transition w-full sm:w-auto mt-2 sm:mt-0 [&>option]:dark:bg-slate-900" onchange="this.form.submit()">
                                ...
                                <?php $__currentLoopData = $marketingSources ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $source): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($source); ?>" <?php echo e(request('marketing_source') === $source ? 'selected' : ''); ?>><?php echo e($source); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
...
<span class="font-medium text-slate-800 dark:text-slate-200">Note:</span> <?php echo e($lead->remarks); ?><?php endif; ?></div>
        ...
        <section x-show="tab === 'notices'" class="grid gap-5 xl:grid-cols-[24rem_1fr]" x-data="{ noticeTargetType: 'all', noticeCourse: '', noticeBatch: '', noticeStudent: '' }">
            <form method="POST" action="<?php echo e(route('computer-training.notices.store')); ?>" enctype="multipart/form-data" class="surface p-5">
                <?php echo csrf_field(); ?>
                ...
                <div class="space-y-3">
                    ...
                    <template x-if="noticeTargetType === 'student'">
                        <select class="field" name="target_student_id" x-model="noticeStudent" required>
                            <option value="">Select Student</option>
                            <template x-for="student in <?php echo e(Js::from(\App\Models\ComputerTrainingStudent::select('id', 'name', 'course', 'batch_id')->get())); ?>" :key="student.id">
                                <option :value="student.id" x-text="student.name"></option>
                            </template>
                        </select>
                    </template>
                    ...
                    <article class="surface p-4">
                        <div class="flex gap-4">
                            <?php if($notice->image_path): ?>
                                <div class="shrink-0">
                                    <img src="<?php echo e(Storage::url($notice->image_path)); ?>" alt="Notice Image" class="w-24 h-24 object-cover rounded shadow-sm">
                                </div>
                            <?php endif; ?>
                            <div class="flex-1 flex justify-between gap-3">
                                <div>
                                    <h3 class="font-semibold text-lg"><?php echo e($notice->title); ?></h3>
                                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400 whitespace-pre-wrap"><?php echo e($notice->body); ?></p>
                                </div>
                                <div class="text-right shrink-0">
                                    <span class="inline-block px-2 py-1 text-xs font-semibold bg-teal-100 text-teal-800 dark:bg-teal-900 dark:text-teal-200 rounded-full mb-2">
                                        <?php if($notice->target_course): ?>
                                            Course: <?php echo e($notice->target_course); ?>

                                        <?php elseif($notice->target_batch_id): ?>
                                            Batch: <?php echo e($notice->batch?->name); ?>

                                        <?php elseif($notice->target_student_id): ?>
                                            Student: <?php echo e($notice->student?->name); ?>

                                        <?php else: ?>
                                            <?php echo e(ucfirst($notice->audience)); ?>

                                        <?php endif; ?>
                                    </span>
                                    <p class="text-sm text-slate-500"><?php echo e($notice->publish_date->format('M j, Y')); ?></p>
                                </div>
                            </div>
                        </div>
                    </article>
